<template>
  <div id="payment">
    ...
    <!-- Echarts -->
    <div class="payment-Echarts">
      <div id="payment-Echarts"></div>
    </div>
    <!-- loading -->
...
export default {
  ...
  methods: {
    ...
    getData() {
      var st1
      this.$axios.get("../../../static/dataJson/payment.json")
        .then(res => {
    ...
    tabData() {
      ...
      if (this.radio == '新增') {
        this.$axios.get("../../../static/dataJson/newpayment.json")
          .then(res => {
            res.data.forEach(item => {
    ...
    payment() {
      // 初始化
      let payment = this.$echarts.init(document.getElementById('payment-Echarts'));
      // 绘图
      payment.setOption({
        // 调色板
...
#payment {
  ...
  #payment-Echarts>div:nth-of-type(1) {
